...
public class CameraController : MonoBehaviour
{
    ...
    public void CameraMovement()
    {
        ...
        if(Input.GetAxis("Mouse ScrollWheel") > 0 && MainCamera.transform.localPosition.z < -5)
        {
            Debug.Log("Колесико вверх");
            MainCamera.transform.localPosition += new Vector3(0, 0, CameraSpeed*Time.deltaTime);
        }
        if(Input.GetAxis("Mouse ScrollWheel") < 0 && MainCamera.transform.localPosition.z > -60)
        {
